...
export function CameraTrackingOverlay({ point, trail, active }: { point: TrackingPoint | null; trail: TrackingPoint[]; active: boolean }) {
  return <div className="tracking-overlay" aria-live="polite">
    ...
    {point && <><span className="tracking-crosshair" style={{ left: `${point.x * 100}%`, top: `${point.y * 100}%` }} /><span className="tracking-label" style={{ left: `${Math.min(78, Math.max(2, point.x * 100))}%`, top: `${Math.min(82, Math.max(4, point.y * 100))}%` }}>Finger region {point.region + 1} · {Math.round(point.confidence * 100)}%</span></>}
