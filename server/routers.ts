...
export function normalizeCompletionMetrics(input: { elapsedMs: number; readingSpeedWpm: number; rereads: number; skippedRegions: number; pauseCount: number; trackingCoverage: number }) {
  return { elapsedMs: Math.max(0, Math.trunc(input.elapsedMs)), readingSpeedWpm: Math.max(0, input.readingSpeedWpm), rereads: Math.max(0, Math.trunc(input.rereads)), skippedRegions: Math.max(0, Math.trunc(input.skippedRegions)), pauseCount: Math.max(0, Math.trunc(input.pauseCount)), trackingCoverage: Math.max(0, Math.min(100, input.trackingCoverage)) };
}
...
export const appRouter = router({
  ...
  reading: router({
    ...
    complete: protectedProcedure.input(z.object({ sessionId: z.number().int().positive(), elapsedMs: z.number().int().nonnegative(), readingSpeedWpm: z.number().nonnegative(), rereads: z.number().int().nonnegative(), skippedRegions: z.number(), pauseCount: z.number().int().nonnegative(), trackingCoverage: z.number().min(0).max(100) })).mutation(async ({ input, ctx }) => { await requireOwnedSession(input.sessionId, ctx.user.id); const normalized = normalizeCompletionMetrics(input); await updateReadingSession(input.sessionId, { ...normalized, status: "completed", completedAt: new Date() }); return getSessionWithEvents(input.sessionId, ctx.user.id); }),
